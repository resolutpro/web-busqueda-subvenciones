import { users, companies, grants, matches, type User, type InsertUser, type Company, type InsertCompany, type Grant, type InsertGrant, type Match, type InsertMatch, type GrantWithMatch, type UpsertUser } from "@shared/schema";
import { db } from "./db";
...
export interface IStorage {
  ...
  // Grants
  getGrants(params?: { search?: string; scope?: string; minAmount?: number }): Promise<Grant[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  // Grants
  async getGrants(params?: { search?: string; scope?: string; minAmount?: number }): Promise<Grant[]> {
  ...
  async getMatch(companyId: number, grantId: number): Promise<Match | undefined> {
  ...
  async generateMatchesForCompany(companyId: number): Promise<void> {
    ...
    for (const grant of allGrants) {
      ...
      if (company.description.toLowerCase().includes('digital') && 
         (grant.title.toLowerCase().includes('digital') || grant.tags?.includes('Digitalizacion'))) {
        score += 30;
      }
      
      // 2. Location Match (20 points)
      // Simplified: If grant is National or matches company location
      if (grant.scope === 'Nacional' || grant.scope === 'Europeo' || 
      ...
      for (const word of keywords) {
        if (word.length > 3 && (grant.title.toLowerCase().includes(word) || grant.rawText?.toLowerCase().includes(word))) {
          keywordMatches++;
        }
      }
      ...
      if (score > 10) {
         await this.createMatch({
           ...
           aiAnalysis: {
             ...
             requirements: ["Estar al corriente con Hacienda", "PYME constituida"] // Mock
           }
         });
      }
    }

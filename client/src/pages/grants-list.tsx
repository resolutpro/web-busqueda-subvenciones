...
import { RefreshCw } from "lucide-react"; // Icono de refresco
...
export default function GrantsListPage() {
  ...
  const scrapeMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/grants"] }); // Refresca la lista
      toast({
        title: "Sincronización exitosa",
        description: "Se han actualizado las subvenciones desde la BDNS.",
      });
    },
    onError: () => {
      toast({
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          >
            ...
            />
            {scrapeMutation.isPending ? "Sincronizando..." : "Sincronizar BDNS"}
          </Button>
        </div>
